...
Status: Draft v1.9 | Last updated: 2026-08-27 | Related: ../AGENTS.md, ../CredChain_Golang/
...
- **Trigger is a plain `<button type="button">` carrying `SelectTrigger`'s class list**, so it is pixel-identical to the `<Select>` fields beside it in the same form grid. `type="button"` matters: inside a `<form>` the default `submit` would post on every open.
